...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Spectro";
        response.header.chatter = "Forbidden: You are not permitted to ping Spectro";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Spectro is up";
        response.header.chatter = "Spectro here. What can I do for you?";
    }
    else {
