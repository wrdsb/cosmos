...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping the MCP";
        response.header.chatter = "Forbidden: You are not permitted to ping the MCP";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "The MCP is up";
        response.header.chatter = "The MCP here. What can I do for you?";
    }
    else {
