...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping ClapTrap";
        response.header.chatter = "Forbidden: You are not permitted to ping ClapTrap";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "ClapTrap is up";
        response.header.chatter = "ClapTrap here. What can I do for you?";
    }
    else {
