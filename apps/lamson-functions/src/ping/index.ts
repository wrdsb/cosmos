...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Lamson";
        response.header.chatter = "Forbidden: You are not permitted to ping Lamson";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Lamson is up";
        response.header.chatter = "Lamson here. What can I do for you?";
    }
    else {
