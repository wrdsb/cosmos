...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Babbage";
        response.header.chatter = "Forbidden: You are not permitted to ping Babbage";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Babbage is up";
        response.header.chatter = "Babbage here. What can I do for you?";
    }
    else {
