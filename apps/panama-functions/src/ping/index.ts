...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Panama";
        response.header.chatter = "Forbidden: You are not permitted to ping Panama";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Panama is up";
        response.header.chatter = "Panama here. What can I do for you?";
    }
    else {
