...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Relay";
        response.header.chatter = "Forbidden: You are not permitted to ping Relay";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Relay is up";
        response.header.chatter = "Relay here. What can I do for you?";
    }
    else {
