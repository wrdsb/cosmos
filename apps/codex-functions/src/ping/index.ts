...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.payload.message = "Forbidden: You are not permitted to ping Codex";
        response.payload.chatter = "Forbidden: You are not permitted to ping Codex";
    }
    else if (authenticated && authorized) {
        response.payload.status = 200;
        response.payload.message = "Codex is up";
        response.payload.chatter = "Codex here. What can I do for you?";
    }
    else {
