...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Codex";
        response.header.chatter = "Forbidden: You are not permitted to ping Codex";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Codex is up";
        response.header.chatter = "Codex here. What can I do for you?";
    }
    else {
