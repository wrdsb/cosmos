...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping the Sorting Hat";
        response.header.chatter = "Forbidden: You are not permitted to ping the Sorting Hat";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "The Sorting Hat is up";
        response.header.chatter = "Sorting Hat here. What can I do for you?";
    }
    else {
