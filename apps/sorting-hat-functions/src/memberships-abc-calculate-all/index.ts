...
import { MembershipsABCCalculateAllFunctionRequest, MembershipsABCCalculateAllFunctionRequestPayload } from "@cosmos/types";

const membershipsABCCalculateAll: AzureFunction = async function (context: Context, triggerMessage: MembershipsABCCalculateAllFunctionRequest): Promise<void> {
    ...
    const schoolCodes = context.bindings.schoolCodes;

    let queueMessages = [];

    schoolCodes.forEach(school => {
        let message = {
    ...
    context.bindings.membershipsABCCalculate = JSON.stringify(queueMessages);
