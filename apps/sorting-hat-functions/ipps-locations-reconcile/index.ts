...
const ippsLocationsReconcile: AzureFunction = async function (context: Context, triggerMessage: string): Promise<void> {
    ...
    const cosmosQuery = "SELECT * FROM c WHERE c.type = 'ipps-location'";
    ...
    context.log('Found ' + Object.getOwnPropertyNames(locations_now).length + ' locations in IPPS.')
    ...
    {
        context.log('Materialize locations...');
        context.log('Materializing ' + Object.getOwnPropertyNames(locations).length + ' locations...')
        ...
        context.log('Materialized ' + Object.getOwnPropertyNames(materializedLocations).length + ' locations.')
