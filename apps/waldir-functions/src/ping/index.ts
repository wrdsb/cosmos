...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping WALDIR";
        response.header.chatter = "Forbidden: You are not permitted to ping WALDIR";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "WALDIR is up";
        response.header.chatter = "WALDIR here. What can I do for you?";
    }
    else {
