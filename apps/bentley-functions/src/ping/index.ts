...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Bentley";
        response.header.chatter = "Forbidden: You are not permitted to ping Bentley";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Bentley is up";
        response.header.chatter = "Bentley here. What can I do for you?";
    }
    else {
