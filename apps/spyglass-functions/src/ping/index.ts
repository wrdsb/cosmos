...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Spyglass";
        response.header.chatter = "Forbidden: You are not permitted to ping Spyglass";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Spyglass is up";
        response.header.chatter = "Spyglass here. What can I do for you?";
    }
    else {
