...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping HAGAR";
        response.header.chatter = "Forbidden: You are not permitted to ping HAGAR";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "HAGAR is up";
        response.header.chatter = "HAGAR here. What can I do for you?";
    }
    else {
