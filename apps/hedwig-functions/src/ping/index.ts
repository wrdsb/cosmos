...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Hedwig";
        response.header.chatter = "Forbidden: You are not permitted to ping Hedwig";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Hedwig is up";
        response.header.chatter = "Hedwig here. What can I do for you?";
    }
    else {
