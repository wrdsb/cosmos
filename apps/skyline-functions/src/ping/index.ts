...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Skyline";
        response.header.chatter = "Forbidden: You are not permitted to ping Skyline";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Skyline is up";
        response.header.chatter = "Skyline here. What can I do for you?";
    }
    else {
