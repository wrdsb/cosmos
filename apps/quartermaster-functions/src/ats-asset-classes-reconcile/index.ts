...
import { FunctionInvocation, ATSAssetClassesReconcileFunctionRequest, ATSAssetClassesReconcileFunctionRequestPayload, ATSAssetClass } from "@cosmos/types";

const atsAssetClassesReconcile: AzureFunction = async function (context: Context, triggerMessage: ATSAssetClassesReconcileFunctionRequest): Promise<void> {
    ...
    const cosmosContainer = 'ats-asset-classes';
    const cosmosClient = new CosmosClient({endpoint: cosmosEndpoint, key: cosmosKey});

    const triggerObject = triggerMessage as ATSAssetClassesReconcileFunctionRequest;
    const payload = triggerObject.payload as ATSAssetClassesReconcileFunctionRequestPayload;
    ...
    context.log('Reconcile ATSAssetClasses: ' + Object.getOwnPropertyNames(recordsPrevious).length);
