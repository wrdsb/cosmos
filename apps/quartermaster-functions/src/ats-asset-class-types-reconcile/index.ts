...
import { FunctionInvocation, ATSAssetClassTypesReconcileFunctionRequest, ATSAssetClassTypesReconcileFunctionRequestPayload, ATSAssetClassType } from "@cosmos/types";

const atsAssetClassTypesReconcile: AzureFunction = async function (context: Context, triggerMessage: ATSAssetClassTypesReconcileFunctionRequest): Promise<void> {
    ...
    const cosmosContainer = 'ats-asset-class-types';
    const cosmosClient = new CosmosClient({endpoint: cosmosEndpoint, key: cosmosKey});

    const triggerObject = triggerMessage as ATSAssetClassTypesReconcileFunctionRequest;
    const payload = triggerObject.payload as ATSAssetClassTypesReconcileFunctionRequestPayload;
    ...
    context.log('Reconcile ATSAssetClassTypes: ' + Object.getOwnPropertyNames(recordsPrevious).length);
    ...
    async function findCreatesAndUpdates(calculation) {
        ...
        Object.getOwnPropertyNames(records_now).forEach(function (record_id) {
            const new_record = {
                ...
                engraved_id_prefix:   records_now[record_id].engraved_id_prefix
            ...
            if (!records_previous || !records_previous[record_id]) {
                ...
                const old_record = {
                    ...
                    engraved_id_prefix:   records_previous[record_id].engraved_id_prefix
            ...
            for (const item of resources) {
                if (!item.deleted) {
                    const dbObject = {
                        ...
                        engraved_id_prefix: item.engraved_id_prefix
...
export default atsAssetClassTypesReconcile;