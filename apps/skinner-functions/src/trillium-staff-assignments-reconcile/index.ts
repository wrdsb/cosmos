...
import { FunctionInvocation, TrilliumStaffAssignmentsReconcileFunctionRequest, TrilliumStaffAssignmentsReconcileFunctionRequestPayload, TrilliumStaffAssignment } from "@cosmos/types";

const trilliumStaffAssignmentsReconcile: AzureFunction = async function (context: Context, triggerMessage: TrilliumStaffAssignmentsReconcileFunctionRequest): Promise<void> {
    const functionInvocation = {
        functionInvocationID: context.executionContext.invocationId,
        functionInvocationTimestamp: new Date().toJSON(),
        functionApp: 'Skinner',
        functionName: context.executionContext.functionName,
        functionDataType: 'TrilliumStaffAssignment',
    ...
    const cosmosContainer = 'staff-assignments';
    const cosmosClient = new CosmosClient({endpoint: cosmosEndpoint, key: cosmosKey});

    const triggerObject = triggerMessage as TrilliumStaffAssignmentsReconcileFunctionRequest;
    const payload = triggerObject.payload as TrilliumStaffAssignmentsReconcileFunctionRequestPayload;
    ...
    context.log('Reconcile staff assignments: ' + Object.getOwnPropertyNames(recordsPrevious).length);
        ...
        try {
            ...
            for (const item of resources) {
                if (!item.deleted) {
                    ...
                    records_previous[item.id] = staffAssignmentObject;
                }
            }
    ...
    function makeHash(objectToHash: TrilliumStaffAssignment): string {
...
export default trilliumStaffAssignmentsReconcile;