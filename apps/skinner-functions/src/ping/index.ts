...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Skinner";
        response.header.chatter = "Forbidden: You are not permitted to ping Skinner";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Skinner is up";
        response.header.chatter = "Skinner here. What can I do for you?";
    }
    else {
