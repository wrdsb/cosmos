...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.payload.message = "Forbidden: You are not permitted to ping Skinner";
        response.payload.chatter = "Forbidden: You are not permitted to ping Skinner";
    }
    else if (authenticated && authorized) {
        response.payload.status = 200;
        response.payload.message = "Skinner is up";
        response.payload.chatter = "Skinner here. What can I do for you?";
    }
    else {
