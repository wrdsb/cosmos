...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    const functionEventType = 'WRDSB.Skinner.Ping';
    ...
    let response = {
        payload: {
            status: 200,
            message: "Skinner is up",
            chatter: "Skinner here. What can I do for you?",
    ...
    if (!authenticated) {
        ...
        context.res = {
            ...
            body: {
                payload: {
                    status: 403,
                    message: "Forbidden: You are not permitted to ping Skinner",
                    chatter: "Forbidden: You are not permitted to ping Skinner",
                    timestamp: functionInvocationTimestamp,
                    authenticated: authenticated,
                    authorized: authorized,
                    roles: JSON.stringify(userRoles)
