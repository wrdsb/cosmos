...
const trilliumStaffReconcile: AzureFunction = async function (context: Context, triggerMessage: TrilliumStaffReconcileFunctionRequest): Promise<void> {
    ...
    const triggerObject = triggerMessage as TrilliumStaffReconcileFunctionRequest;
    const payload = triggerObject.payload as TrilliumStaffReconcileFunctionRequestPayload;
    ...
    context.log('Reconcile staff: ' + Object.getOwnPropertyNames(recordsPrevious).length);
    ...
    async function findCreatesAndUpdates(calculation) {
        ...
        Object.getOwnPropertyNames(records_now).forEach(function (record_id) {
            const new_record = {
                ...
                school_year:         records_now[record_id].school_year,
                staff_type:          records_now[record_id].staff_type,
            ...
            if (!records_previous || !records_previous[record_id]) {
                ...
                const old_record = {
                    ...
                    school_year:         records_previous[record_id].school_year,
                    staff_type:          records_previous[record_id].staff_type,
        ...
        try {
            ...
            for (const item of resources) {
                if (!item.deleted) {
                    ...
                    records_previous[item.id] = staffObject;
                }
            }
