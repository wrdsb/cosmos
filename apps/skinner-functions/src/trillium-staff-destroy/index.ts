...
import { FunctionInvocation, TrilliumStaffDestroyFunctionRequest, TrilliumStaffDestroyFunctionRequestPayload, TrilliumStaff } from "@cosmos/types";

const trilliumStaffStore: AzureFunction = async function (context: Context, triggerMessage: TrilliumStaffDestroyFunctionRequest): Promise<void> {
    const functionInvocation = {
        ...
        functionDataOperation: 'Destroy',
        eventLabel: ''
    } as FunctionInvocation;

    const triggerObject = triggerMessage as TrilliumStaffDestroyFunctionRequest;
    const operation = triggerObject.operation;
    const payload = triggerObject.payload as TrilliumStaffDestroyFunctionRequestPayload;
    ...
    switch (operation) {
        case 'destroy':
            result = doDestroy(payload);
            statusCode = '200';
            statusMessage = 'Success: Staff record destroyed.';
            break;
        default:
            break;
    }
