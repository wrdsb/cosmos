...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Tollbooth";
        response.header.chatter = "Forbidden: You are not permitted to ping Tollbooth";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Tollbooth is up";
        response.header.chatter = "Tollbooth here. What can I do for you?";
    }
    else {
