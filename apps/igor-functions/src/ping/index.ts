...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping IGOR";
        response.header.chatter = "Forbidden: You are not permitted to ping IGOR";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "IGOR is up";
        response.header.chatter = "IGOR here. What can I do for you?";
    }
    else {
