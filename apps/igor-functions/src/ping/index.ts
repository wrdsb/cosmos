...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.payload.message = "Forbidden: You are not permitted to ping IGOR";
        response.payload.chatter = "Forbidden: You are not permitted to ping IGOR";
    }
    else if (authenticated && authorized) {
        response.payload.status = 200;
        response.payload.message = "IGOR is up";
        response.payload.chatter = "IGOR here. What can I do for you?";
    }
    else {
