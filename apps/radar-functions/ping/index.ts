...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping RADAR";
        response.header.chatter = "Forbidden: You are not permitted to ping RADAR";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "RADAR is up";
        response.header.chatter = "RADAR here. What can I do for you?";
    }
    else {
