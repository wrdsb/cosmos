...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Friday";
        response.header.chatter = "Forbidden: You are not permitted to ping Friday";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Friday is up";
        response.header.chatter = "Friday here. What can I do for you?";
    }
    else {
