...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.payload.message = "Forbidden: You are not permitted to ping Houston";
        response.payload.chatter = "Forbidden: You are not permitted to ping Houston";
    }
    else if (authenticated && authorized) {
        response.payload.status = 200;
        response.payload.message = "Houston is up";
        response.payload.chatter = "Houston here. What can I do for you?";
    }
    else {
