...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Houston";
        response.header.chatter = "Forbidden: You are not permitted to ping Houston";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Houston is up";
        response.header.chatter = "Houston here. What can I do for you?";
    }
    else {
