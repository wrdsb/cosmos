...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Crankcase";
        response.header.chatter = "Forbidden: You are not permitted to ping Crankcase";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Crankcase is up";
        response.header.chatter = "Crankcase here. What can I do for you?";
    }
    else {
