...
const ping: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (!authenticated) {
        ...
        response.header.message = "Forbidden: You are not permitted to ping Flenderson";
        response.header.chatter = "Forbidden: You are not permitted to ping Flenderson";
    }
    else if (authenticated && authorized) {
        response.header.status = 200;
        response.header.message = "Flenderson is up";
        response.header.chatter = "Flenderson here. What can I do for you?";
    }
    else {
