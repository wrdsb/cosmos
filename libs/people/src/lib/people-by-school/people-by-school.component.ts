...
import { PeopleService } from '@cosmos/search-services';
import { map, debounceTime, distinctUntilChanged } from 'rxjs/operators';
import { PersonMetaDialogComponent } from "../person-meta-dialog/person-meta-dialog.component";

@Component({
  selector: 'cosmos-people-by-school',
  templateUrl: './people-by-school.component.html',
  styleUrls: ['./people-by-school.component.scss']
})
export class PeopleBySchoolComponent implements OnInit {
  columns = [
    //"id",
    //"createdAt",
    //"updatedAt",
  ...
  schoolCodeSearchFormControl = new FormControl();

  currentSchoolCodeSearch = new BehaviorSubject<string>('');
  ...
  constructor(
    ...
    this.searchRequestState$ = this.peopleService.searchRequestState$;
    this.searchResponse$ = this.peopleService.searchResponse$;
    ...
    this.peoplePage$ = this.peopleService.itemsList$;

    this.personSelected$ = this.peopleService.itemSelected$;
    this.selectedPerson$ = this.peopleService.selectedItem$;
  }
  ...
  ngAfterViewInit() {
    this.schoolCodeSearchFormControl.valueChanges
      .pipe(
        debounceTime(400),
      ...
      .subscribe(searchTerm => {
        ...
        this.currentSchoolCodeSearch.next(searchTerm);
  ...
  selectPerson(groupID: string): void {
    ...
    this.peopleService.selectItem(groupID);
  ...
  deselectGroup(): void {
    ...
    this.peopleService.deselectItem();
  }
  ...
  searchPeople(): void {
    ...
    if (this.currentSchoolCodeSearch.value.length > 0) {
      searchString = this.currentSchoolCodeSearch.value;
    }
    ...
    this.peopleService.searchPeople(searchRequestPayload);
  }
}
